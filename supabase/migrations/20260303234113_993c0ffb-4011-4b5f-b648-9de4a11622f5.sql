...
-- FIX CRITICAL RLS BUG: All policies were RESTRICTIVE (ALL must pass) instead of PERMISSIVE (ANY can pass)
-- This blocked ALL data access for non-admin users since they can't satisfy admin + partner policies simultaneously
...
CREATE POLICY "Venue partners can manage own locations" ON public.locations FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'venue_partner'::app_role) AND organization_id IN (SELECT profiles.organization_id FROM profiles WHERE profiles.id = auth.uid()))
  WITH CHECK (public.has_role(auth.uid(), 'venue_partner'::app_role) AND organization_id IN (SELECT profiles.organization_id FROM profiles WHERE profiles.id = auth.uid()));
...
CREATE POLICY "Nonprofit users read by profile" ON public.nonprofits FOR SELECT TO authenticated
  USING (id IN (SELECT profiles.nonprofit_id FROM profiles WHERE profiles.id = auth.uid()));
...
DROP POLICY IF EXISTS "Nonprofit partners insert own locations" ON public.nonprofit_locations;
...
CREATE POLICY "Nonprofit partners manage own locations" ON public.nonprofit_locations FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'nonprofit_partner'::app_role) AND nonprofit_id IN (SELECT nonprofits.id FROM nonprofits WHERE nonprofits.user_id = auth.uid()))
  WITH CHECK (public.has_role(auth.uid(), 'nonprofit_partner'::app_role) AND nonprofit_id IN (SELECT nonprofits.id FROM nonprofits WHERE nonprofits.user_id = auth.uid()));

CREATE POLICY "Nonprofit users read assigned locations" ON public.nonprofit_locations FOR SELECT TO authenticated
  USING (id IN (SELECT profiles.nonprofit_location_id FROM profiles WHERE profiles.id = auth.uid()));
