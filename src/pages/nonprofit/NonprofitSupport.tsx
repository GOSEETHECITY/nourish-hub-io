...
export default function NonprofitSupport() {
  ...
  const { data: nonprofit } = useQuery({
    queryKey: ["np-name", profile?.nonprofit_id],
    queryFn: async () => {
      const { data } = await supabase.from("nonprofits").select("organization_name").eq("id", profile!.nonprofit_id!).single();
      return data;
    },
  ...
  const submit = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("support_requests").insert({
        ...
        organization_name: nonprofit?.organization_name || null,
      });
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-card rounded-xl border p-5 space-y-4">
        <div className="space-y-1.5">
          ...
          <Input id="np-subject" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="What is this about?" />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="np-message">Message</Label>
          <Textarea id="np-message" rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Tell us what you need help with." />
        </div>
        <Button
