...
export default function GovernmentSettings() {
  ...
  const { data: org } = useQuery({
    queryKey: ["gov-org", profile?.organization_id],
    queryFn: async () => {
  ...
  const regions = org?.government_regions as any;

  return (
    <div className="space-y-8 max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Settings</h1>
        <p className="text-sm text-muted-foreground mt-1">Profile and account settings</p>
      </div>
      ...
      {org && (
        <section className="bg-card rounded-xl border p-6 space-y-3">
          ...
          {regions && (
            <div>
              <p className="text-xs text-muted-foreground uppercase">Assigned Region</p>
              <p className="text-sm text-foreground mt-1">
                {regions.is_state_wide ? `Statewide: ${regions.state}` : regions.cities?.length ? `Cities: ${regions.cities.join(", ")}` : regions.counties?.length ? `Counties: ${regions.counties.join(", ")}` : "All"}
              </p>
            </div>
          )}
        </section>
