...
export default function GovernmentDashboard() {
  ...
  const { data: locs = [] } = useQuery({
    queryKey: ["gov-locs"],
    queryFn: async () => {
      const { data, error } = await supabase.from("locations").select("id, organization_id, city, state");
      if (error) throw error;
      return data as any[];
    },
  });
  ...
  }, [listings, filterCity, filterState, locMap]);
          ...
          <Table>
            ...
            <TableBody>
              {filteredListings.slice(0, 20).length === 0 ? (
                <TableRow><TableCell colSpan={5} className="text-center py-12 text-muted-foreground">No food listings found.</TableCell></TableRow>
